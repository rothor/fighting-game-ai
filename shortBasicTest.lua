...
function sbt_calculateScore(p1Hp, p2Hp, timeLeft)
	return (2880 - p2Hp) + p1Hp
end

function sbt_runSingleTest(saveState, t)
		-- initialization
	...
	while true do
		...
		if p1Hp == 0 or p2Hp == 0 or timeLeft == 0 or timeLeft <= 98 - 5 * 3 then
			client.pause()
			return {sbt_calculateScore(p1Hp, p2Hp, timeLeft), false}
		end
...
function sbt_runTests(t)
	local sstate = {5, 6, 7, 8, 9, 0}
	local score = {0, 0, 0, 0, 0, 0}
	for i = 1, 1, 1 do
		local tempScore = sbt_runSingleTest(sstate[i], t)
